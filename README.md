...
0.6.0 rebuilds the workflow on top of **advisory-only hooks** — every
...
- **Four advisory hooks (`exit 0`, never block):** SessionStart,
  UserPromptSubmit, Stop, SessionEnd. Together they manage the
  session lifecycle, inject the right selector prompt at each
  phase-gate node, remind the model to keep docs and code in sync,
...
plugins/specode/
  ...
  hooks/hooks.json                ← 4 advisory hook handlers
