...
plugins/specode/
  ...
  hooks/                       ← v0.6 引入 4 个 hook（SessionStart / UserPromptSubmit / Stop / SessionEnd）；
                                  v0.7 加 PostToolUse Task；v0.8 加 heartbeat + PreToolUse tasks.md guard
    hooks.json
  scripts/                     ← v0.6 引入 spec_* 系列；v0.7 引入 task_swarm_*
    run.sh
    run.cmd
    spec_vault.py              ← v0.6
    spec_init.py               ← v0.6
...
| 文档确认 / 任务执行 / 接管 / 验收 selector | `references/prompts.md` | 同上 | 完整迁移；任务执行 selector v0.7 加 "task-swarm 多 agent 并发"；统一格式见 §3.7 |
...
| **任务执行** | A 单选 | tasks.md 确认后 | 任务执行选择 | 开始 required / 开始 required + optional / 用 task-swarm 多 agent 并发（v0.7 加）/ 暂不 coding |
...
##### (8) iteration-scope （类型 C 复选；v0.7+ 启用，v0.6 占位）
...
[requirements/bugfix] ─文档确认─► [design] ─文档确认─► [tasks]
                                       ...
                                       ├─ 用 task-swarm 多 agent 并发 ◄── v0.7 加
                                       └─ 暂不 coding
                                                        │
                                                        ▼
...
- **tasks.md 写回**：永远走 `task_swarm.py writeback`。CLI 严格 diff——只允许 checkbox toggle + `> ` 注释块。v0.8 引入的 `on-pre-tool-use` hook 在主会话直写 tasks.md 时**提醒**（不阻断）。
...
### 5.1 `SessionStart` `on-session-start`（v0.6 引入）
...
### 5.2 `UserPromptSubmit` `on-user-prompt`（v0.6 引入）
...
### 5.4 `SessionEnd` `on-session-end`（v0.6 引入）
...
### 5.5 `PostToolUse` `on-task-completed`（v0.7 引入）
...
### 5.6 `UserPromptSubmit` `on-heartbeat-quiet`（v0.8 引入）
...
### 5.7 `PreToolUse` `on-pre-tool-use`（v0.8 引入）
