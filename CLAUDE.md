...
- **Claude Code and CodeBuddy are supported/verified. Kimi local install is verified** — pluginhub cannot be installed from a URL (Kimi does not support monorepo subdirectory installs), only from a local clone; its `SessionStart` advisory is injected via each manifest's `sessionStart.skill` (`using-specode` / `using-ragkit`), not a hook. **Codex schema is conformed to its official docs** (`.agents/plugins/marketplace.json` entries use `source: {source: "local", path: "./plugins/<name>"}` and support subdirectory installs; the install verb is `codex plugin add`, not `install`; the `SessionStart` hook uses `${PLUGIN_ROOT}`; task-swarm subagents dispatch via `spawn_agent` and require `multi_agent = true` in `~/.codex/config.toml`, and `agents/*.md` are not loaded by Codex so per-role tool isolation is by prompt) **but is not yet run on a real host**. See README §Multi-host support for the full unverified list.
