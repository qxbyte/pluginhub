...
**Recent BREAKING context (read before touching memory/knowledge code):** specode 4.0.0 + task-swarm 0.10.0 **ripped out the whole memory-injection engine** — no more codemap recall, rule-check, auto-distill, `_ingest_lessons.py`, or `.ai-memory/knowledge/*.yml` writes. Round 1/2 baseline experiments showed the recall round-trip did not net-save tokens. `distill` was rewritten into a **manual, md-only Obsidian organizer** (default output `/Volumes/External HD/Obsidian/Notes/11-KnowledgeBase/<slug>/`). The v3 behavior lives at tag/branch `backup/specode-v3.4.0-task-swarm-v0.9.2`. Do not reintroduce auto-injection without an explicit ask.
...
| `scripts/` member | Role |
|---|---|
| `resolve_root.py` | The only business CLI. specsRoot resolution + persistence + spec listing. stdlib-only. Invoked from the `commands/*.md` command files via `run.sh`. |
...
- **CHANGELOG.md** (root) + each **plugins/<name>/CHANGELOG.md** — narrative history. The big recent inflection is specode 4.0.0 / task-swarm 0.10.0 ripping out the memory-injection engine; older entries cover the pre-1.0.0 heavy state machine and task-swarm's M1 extraction. Branch `backup/specode-v3.4.0-task-swarm-v0.9.2` preserves the v3 memory-injection code.
...
- **plugins/specode/skills/distill/SKILL.md** — authoritative behavior for the off-pipeline distill command.
