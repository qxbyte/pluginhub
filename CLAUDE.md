...
The repo root holds: the four per-host marketplace catalogs (`.claude-plugin/marketplace.json` + `.codebuddy-plugin/marketplace.json` + `.agents/plugins/marketplace.json` + `.kimi-plugin/marketplace.json`, each listing **all four** plugins), `scripts/check_marketplace_versions.py` (the version-sync gate — validates all four hosts are in lock-step, see below), the English README (root) and `docs/` (the zh-CN README, the root CHANGELOG, and CONTRIBUTING — moved there 2026-08; `docs/superpowers/` is gitignored). Plugin internals are listed in README §Architecture — do not re-derive the file tree, read the README.
...
- **Claude Code and CodeBuddy are supported/verified. Codex install + skills are verified on a real host** (subdirectory install via `codex plugin marketplace add` + `codex plugin add` works; the `SessionStart` hook advisory is not yet confirmed). **Kimi local install is verified** — pluginhub cannot be installed from a URL (Kimi does not support monorepo subdirectory installs), only from a local clone; its `SessionStart` advisory is injected via each manifest's `sessionStart.skill` (`using-specode` / `using-ragkit`), not a hook. Codex specifics: `.agents/plugins/marketplace.json` entries use `source: {source: "local", path: "./plugins/<name>"}`; the install verb is `codex plugin add`, not `install`; the `SessionStart` hook uses `${PLUGIN_ROOT}`; task-swarm subagents dispatch via `spawn_agent` and require `multi_agent = true` in `~/.codex/config.toml`, and `agents/*.md` are not loaded by Codex so per-role tool isolation is by prompt. See README §Multi-host support for the remaining unverified list.
...
### The 4 fixed documents (硬约束)
Whatever the execution engine, a spec **always** produces exactly these 4, with **fixed filenames**, **fixed location** `<specsRoot>/<slug>/`:
...
Each planning document (`requirements.md`, `design.md`, `tasks.md`) has a blocking approval boundary: after landing the document, report its path and summary, ask the canonical approval question, and end the turn. Only explicit approval advances the phase; requested edits update the same document and reopen the same gate. Approval is conversational, not persisted as extra state.
...
- **README.md** (root) / **docs/README.zh-CN.md** — what each plugin does, install/usage, the lite architecture map. Keep both in sync on release.
- **docs/CONTRIBUTING.md** — the full version of the conventions above (stdlib rule, CLI wrapper contract, hook advisory rule, hermetic test conventions, release).
- **docs/CHANGELOG.md** (root changelog) + each **plugins/<name>/CHANGELOG.md** — narrative history. Recent inflections: specode 4.0.0 / task-swarm 0.10.0 ripped out the memory-injection engine; **specode 5.1.0 re-introduced retrieval/injection on a 定位用 (pointers-not-facts) basis** (`knowledge.py` + project `knowledge-base/` + `references/retrieval.md`). Older entries cover the pre-1.0.0 heavy state machine and task-swarm's M1 extraction. Branch `backup/specode-v3.4.0-task-swarm-v0.9.2` preserves the v3 memory-injection code.
