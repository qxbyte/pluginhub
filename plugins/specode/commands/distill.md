...
`/specode:distill <slug>` — 沉淀单个 spec 的知识到该 spec 自己的 `project_root`。
...
行为：调用 `distill` skill 走 5 步流程（解析 project_root → 准备目录 → 读 spec 全文 → AskUserQuestion 拆分提议 → 写 yml + md 双产物）。详见 `skills/distill/SKILL.md`。
