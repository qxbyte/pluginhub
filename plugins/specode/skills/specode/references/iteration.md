...
- SKILL.md 不自动呈现 `iteration-scope` 选择器 —— v0.6 默认进入 iteration 后停在 chat 等用户提下一步。
- v0.7+ 起，若用户明示"开始下一轮迭代" → 呈现 `iteration-scope`（类型 C）让用户选本轮调整范围。
...
- 只有用户明确通过 `acceptance-gate=验收通过` 或后续 `/specode:iterate`（如果将来引入）才被改写为 `iteration`。
- 这样可避免把还在 `design` 阶段的 spec 误置为 iteration、跳过未完成的设计门控。

## 7. v0.7+ `iteration-scope` 选择器（类型 C）

v0.7 起，iteration 子循环开始时主会话呈现 `iteration-scope`（类型 C 复选框）让用户勾选本轮调整范围。详见 `references/prompts.md` §(8)。
...
v0.6 该选择器只是占位，**不**自动呈现 —— 默认在 chat 等用户提出下一步。
...
- phase 序列与 `acceptance-gate` 选择器详情 → `references/workflow.md` §7、`references/prompts.md` §(7)。
- 6 份文档的章节模板 → `references/templates.md`。
- 锁状态机（iteration 期间仍受锁保护）→ `references/lock-protocol.md`。
- 入口选择器 `iteration-scope`（v0.7+）→ `references/prompts.md` §(8)。
