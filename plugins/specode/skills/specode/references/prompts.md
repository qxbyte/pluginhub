...
每个 phase-gate 节点必须输出**结构化文本选择器**（不是自由叙述）。本文件给出 v0.6 全部 8 个固定场景的标题、选项标签、推荐项、触发 phase；以及三种类型的统一文本骨架。
...
**v0.6 仅 `iteration-scope` 一个场景占位**（v0.7 启用）；其他 phase-gate 不用类型 C。
...
### (8) `iteration-scope` — 类型 C（v0.7+ 启用；v0.6 占位）
