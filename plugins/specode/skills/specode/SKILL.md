...
实际反例：选定 `doc-confirm-design` 后切到 tasks phase，hook 下一轮会注入 `tasks-execution` selector 模板（4 个固定选项：用 task-swarm / 顺序执行 / 需要调整 / 暂不 coding）；如果同一 turn 内主代理主动呈现，**必须查 `_selectors.py` SELECTOR_PROMPTS['tasks-execution']** 拿到 4 个固定选项原文，不要 invent 成 2 选项简化版。
