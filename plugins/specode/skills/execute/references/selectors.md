---
description: Use when tasks.md is complete and you need to present the "execution mode" selector to the user — verbatim AskUserQuestion example for the specode execute skill's single fixed selector.
---

# Execution mode selector (the one fixed selector)

After tasks.md is confirmed, call `AskUserQuestion` with **adaptive option assembly** (include only options for engines that are installed, up to 4; if none are installed, only the specode self-execute option remains and you may skip the selector and proceed directly). Pass label/description verbatim — do not translate or simplify:

- question: "tasks 已完成。怎么执行？"
- header: "执行方式"
- multiSelect: false
- options (trimmed based on what is installed):
  - label: "委托 task-swarm（多 agent 并发）"
    description: "需已装 task-swarm。读 tasks.md Task + Files 生成 pipeline.yml，过目后并发执行。"
  ...
  - label: "specode 自执行（顺序单 agent）"
    description: "都没装时的降级。主代理按 tasks.md Task 直接 TDD + 自验。"
