...
**目的**：design.md 已生成 / 更新；让用户确认是否进入 delegated phase（委托执行），
或者先看全文 / 继续修改。
...
- 选 "确认" → 调 `phase-transition --from design --to delegated`（**不生成 tasks.md**；M4 起任务拆分归 task-swarm 的 planner，由主代理在委托时读 `design.md` 生成 `pipeline.yml`）→ 立即呈现 `delegation` selector
...
""",
    "delegation": """## 选择器节点：委托执行选择

**目的**：design 已完成；让用户选择如何执行——委托独立 task-swarm plugin（多 agent 并发）、specode 自执行（顺序单 agent）、或回退调整 design。M4 起 tasks/implementation 合并为 delegated 阶段：任务拆分归 task-swarm 的 planner（主代理读 design 生成 pipeline.yml），specode 不再单独产 tasks.md。

**上下文**：active spec=<slug>，phase=delegated。

**前置动作（chat 简报，≤6 行）**：
- 一句话点出 `design.md` 覆盖的主要工作面
- 提示「委托」选项需已安装 task-swarm plugin（主代理不确定是否安装时，如实告知用户）
...
questions:
  - question: "design 已完成。怎么执行？"
    ...
    multiSelect: false
    options:
      - label: "委托 task-swarm 执行（多 agent 并发）"
        description: "需已安装 task-swarm plugin。主代理按 task-swarm SKILL 读 `design.md` 生成 `pipeline.yml`（先给你过目）→ init --pipeline → 多组并发 coder + 每组 reviewer/validator。"
      - label: "specode 自执行（顺序，单 agent）"
        description: "不依赖 task-swarm。主代理按 `design.md` 逐步实现 + 自验，[ ] → [x] 推进。适合小改动或未装 task-swarm。"
      - label: "暂停 / 调整 design"
        description: "design 不符合预期需要调整，或暂不开始（Other 输入说明具体哪种）。"

**约束**：
- 3 个选项；细化需求走 "Other" 输入。
- 调用工具后立即 end turn。
- 简报必须在工具调用**之前**输出。

**用户选定后流程（同一 turn 内继续）**

- 选 "委托 task-swarm 执行（多 agent 并发）" → 按 SKILL.md §「Delegated / 委托执行」：Read task-swarm SKILL → 读本 spec `design.md` 生成 `<spec-dir>/pipeline.yml` → 呈现 yml 摘要（任务组 / @writes / needs 拓扑）给用户过目 → `task_swarm.py init --pipeline <yml> --workdir <项目根> --spec-id <slug> --spec-dir <spec-dir> --session <id>` → 调 `spec_session.py set-delegated-run --run-id <rid>` → 按 task-swarm SKILL 驱动 plan→fork→advance→writeback→resolve → resolve done 后 `set-delegated-run --clear` + `phase-transition --from delegated --to acceptance`。**未装 task-swarm（init 调用失败）** → 告知用户安装，或改选 specode 自执行。
- 选 "specode 自执行（顺序，单 agent）" → 按 SKILL.md §「Delegated / 自执行」：按 `design.md` 逐步 Edit 代码 + 自验 → 完成后 `phase-transition --from delegated --to acceptance`。
- 选 "暂停 / 调整 design" → end turn 等用户反馈：若是调整 → 下一 turn Edit `design.md` → 重新呈现本 selector；若是暂停 → 留在 delegated phase，随时 `/specode:end` 退出或后续 `/specode:continue <slug>` 续接
""",
...
- 勾 "改 tasks" → `--to delegated` → 重新委托 task-swarm（或 specode 自执行）→ 呈现 `delegation`
- 勾 "重跑测试" → 留 iteration phase，执行 `design.md` 的测试要点 / task-swarm `report` 中的复现命令 + 报告结果
