...
## 0.12.2 (2026-07-21) — Codex 子代理适配说明（spawn_agent + multi_agent）

随 pluginhub 的 Codex 安装适配修正同发：swarm SKILL 的 Host-tool convention 补一段
**Codex 子代理差异**——Codex 派子代理是 `spawn_agent` / `wait_agent` / `close_agent`，
需在 `~/.codex/config.toml` 开 `multi_agent = true`（否则无派发工具→降级顺序单代理）；
`agents/*.md` 是 Claude/CodeBuddy agent 格式，**Codex 不加载**，故 reviewer/validator
的「无 Edit/Write」隔离在 Codex 上靠 prompt 约束、不是工具层强制。仓库层根
`.agents/plugins/marketplace.json` 同步改为 Codex 官方 schema（每条
`source: {source: local, path: ./plugins/<name>}` + `policy`；Codex 支持 monorepo
子目录安装）。仍未真机验证。

## 0.12.1 (2026-07-21) — 修 Kimi 市场 schema + Kimi 工具映射

随 pluginhub Kimi 安装修复同发：`.kimi-plugin/marketplace.json` 改 Kimi 官方 schema（`version:"2"`/`id`/`source` 指向各插件子目录，本地 clone 可装）；`.kimi-plugin/plugin.json` 加 `skillInstructions`——把「fork coder/reviewer/validator」「dispatch subagent」映射到 Kimi 的 `Agent` 工具 + `subagent_type`（coder/explore/plan），并重申「子代理完成以宿主确认为准」。task-swarm 无 SessionStart advisory，故不加 `sessionStart`。README Kimi 安装说明改为本地 clone。仍未真机验证。

## 0.12.0 (2026-07-20) — 多宿主适配：编排语义去宿主绑定 + CodeBuddy/Codex/Kimi 独立 manifest

task-swarm 现在同时面向 Claude Code / CodeBuddy / Codex / Kimi 四个宿主。

- **编排语义去宿主绑定**：`skills/swarm/SKILL.md` 与 `references/task-swarm.md` 顶部各加一条「Host-tool convention」——「fork」= 通过宿主的子代理机制派发子代理，`Task`/`Agent` 工具、`Explore` 只读调查代理、teammates UI 运行状态展示这些名字**照旧保留**（对有它们的 Claude 系宿主最可靠），并说明兜底：宿主无子代理能力时降级为顺序单代理执行；子代理「完成」永远以宿主确认其派发任务成功返回为准，不认口头汇报。正文里把「⠙ streaming / ⠴ running Bash」等纯 UI spinner 措辞泛化为「宿主的子代理运行状态展示」。
- **注入子代理的开发纪律文案去 Claude 绑定**：`_prompt.py` 把「Claude Code subagent 无 Skill tool 权限」泛化为「被派发的子代理在当前架构下通常无 skill 调用权限（各宿主一致，因子代理是沙箱执行者）」，`Skill(...)` 保留为示例并补「或等价的 skill 调用」。
- **新增三宿主独立 manifest**：`.codebuddy-plugin/plugin.json` + `.codex-plugin/plugin.json` + `.kimi-plugin/plugin.json`（本插件无 hooks，故不含 hooks 字段），及三份根 catalog。**注**：task-swarm 的 `agents/*.md`（coder/reviewer/validator）是 Claude 系 agent 格式（frontmatter `tools`/`model`），CodeBuddy 同格式可用；Codex 的自定义子代理是 TOML 格式，本轮**未转换**，Codex 下多 agent 隔离能力待实测（见 README 待实测标注）。
- **修 reviewer 脏工作树误报边界违规（真机测试发现）**：此前 reviewer 靠裸 `git status` 判断「coder 改了哪些文件」，会把 run **开始前就已存在**的未提交改动误判为 coder 越界写 `@writes` 之外，产生假 `[contract]` P0 并升级到 p0-fix 去改 coder 从未碰过的文件。修法：`init` 用 `git status --porcelain` 快照 `project_root` 的预存 dirty 集，存入 `state.json` 的 `preexisting_dirty`；reviewer 的 task.md 在「变更归属」段列出该集并要求**以 coder result.md 为准**、排除预存改动。非 git 仓库/git 缺失时优雅降级（空集，退回 result.md 归属）。新增 `test_reviewer_change_attribution.py`（5 测）。
- 256 测试（specode 77 + task-swarm 179）全绿；命令名 `/task-swarm:swarm` 不变 → 兼容 **minor**。

## 0.11.0 (2026-07-08) — 命令 skill 化：/task-swarm:swarm 由 skill 直接提供 + resolver 回归相对路径

- **command → skill**：`commands/swarm.md` 的脚本调用逻辑（resolver + init/plan/fork/advance/writeback/resolve/report/heartbeat CLI）全部并入 skill，删除整个 `commands/` 目录。skill 目录 `skills/task-swarm/` → **`skills/swarm/`**（`git mv` 保历史），frontmatter `name: task-swarm` → `name: swarm`、去掉 `user-invocable: false`——`/task-swarm:swarm` 命令名不变（plugin:skill = task-swarm:swarm），但改由 **user-invocable skill 直接提供、直达 skill 层**。specode 通过 `/task-swarm:swarm` 的 zero-import 调用不受影响。
- **resolver 回归 superpowers 相对路径范式**：所有 `task_swarm.py` 调用从 `R="${CLAUDE_PLUGIN_ROOT:-$CODEBUDDY_PLUGIN_ROOT}"; find ...` 改为 `sh ../../scripts/run.sh ../../scripts/task_swarm.py`（裸相对，靠 skill base directory 拼绝对）。根治 Windows 下 `:-` 被 host 吞空、CodeBuddy 不注入变量（与 ragkit 0.1.7 / specode 6.2.0 同源）。
- **skill description 朴素化**：去掉双引号 + 半角 `Trigger: `（冒号+空格），避免 CodeBuddy YAML 解析失败致 skill 坍缩（同 ragkit 0.1.6 的坑）。
- 冒烟测试适配新结构（skill 路径 `skills/swarm/`、`commands/` 已删断言）；174 测试全绿、`claude plugin validate` 通过。命令名 `/task-swarm:swarm` 不变 → 兼容 **minor**。

## 0.10.5 (2026-07-08) — 修 skill frontmatter YAML 解析失败（P0）

- 0.10.4 的主编排 skill(`skills/task-swarm/SKILL.md`)`description` 含**半角 `: `（冒号+空格）**（如 `Trigger: /task-swarm:swarm`）且未加引号,导致 **YAML frontmatter 解析失败**——`claude plugin validate` 报错,且运行时 frontmatter 被整体静默丢弃(name/description 全丢)。与全角符号无关。修法:description 加双引号并转义内部引号。零行为变化。

## 0.10.4 (2026-07-06) — skill description 统一模板

- skill `description`（渐进式加载唯一常驻的元数据）统一为「Use when 场景 → 做什么 → Trigger 触发词/命令 → 边界/谁触发」轻量模板，`Trigger words —` 标签规范为 `Trigger:`，并补上「specode 执行阶段委托至此」的触发场景。零行为变化。
...
## 0.10.1 (2026-06-30) — 斜杠菜单：隐藏裸 `/task-swarm`

### Changed

- 编排内核 skill `task-swarm`（`skills/task-swarm/SKILL.md`）加 `user-invocable: false` —— 裸 `/task-swarm` 不再出现在斜杠菜单（应经 `/task-swarm:swarm` 命令或 trigger words 激活，Claude 仍可自动调用）。命令 `/task-swarm:swarm` 不变。
- 跨插件引用同步：`cli.py` 注释里的 `/specode:specode-distill` → `/specode:distill`（specode 5.0.0 命令改名）。

非破坏性：无命令 / CLI / 文件协议变更。

## 0.10.0 (2026-06-29) — BREAKING: 拔出记忆注入工程

Round 1/2 baseline 控制变量实验 (`/Volumes/External HD/Obsidian/Notes/07-Ideas/AI-Enterprise-Delivery-System/基线AB对照实验/`) 证明: 记忆检索注入 (codemap recall + auto ingest_lessons + auto distill) 整体未 net 节省 token。3 case 累加 A (有 recall) 比 B (无 recall) 多 1.3% token。recall 价值仅集中在 "可直接复用代码" 的 coder 阶段 (case 6 g1.1 -88% token), 但其他阶段 (reviewer/validator/redundant context) 累计反向 cost 抵消。

→ 用户决策: 完全拔出记忆注入工程, task-swarm 专注 "多 agent 编排 + 文件协议" 本质能力。

### Removed

- **`_ingest_lessons.py`** 文件删除 (含 _build_case_fields / _build_pit_fields / _codemap_write 等所有逻辑)
- **`tests/test_ingest_lessons.py`** 文件删除
- **`cmd_resolve` 自动 ingest** 调用 (line 1475-1497) 删除
- **`--no-ingest` flag** (resolve + run-loop subcommand 各一处) 删除
- **`resolve_args.no_ingest`** kwarg (cmd_run_loop 调 cmd_resolve 时) 删除
- **state.json events** 中 `ingest-lessons` 事件不再生成
- **`emit()` 输出** 不再含 `ingest: {cases, pitfalls, skipped}` 字段

### Behavior changes

- `task_swarm.py resolve --run <id>`: 仅做 lifecycle 终态收尾 (mark `failed_status=done`, write events, save sm), 不写任何 `.ai-memory/knowledge/*.yml`
- `task_swarm.py run-loop --run <id>`: 同上, 触发 resolve 也不写
- `<project_root>/.ai-memory/knowledge/` 完全由 task-swarm 不再触碰 (历史 ingest 的文件保留, 不删)

### Migration

- 如需 per-spec md 知识到 Obsidian wiki: 手动跑 `/specode:specode-distill <slug>` (specode 4.0.0+, md-only)
- 如需保留 v0.9.x ingest 自动行为: `git checkout backup/specode-v3.4.0-task-swarm-v0.9.2`
- codemap-aimemory dependency removed: task-swarm 0.10.0 不再要求 codemap-aimemory 安装

### Tests
- 174/174 task-swarm 测试全 pass (含 `test_runloop_and_pipeline_end_logic.py` 移除 `--no-ingest` arg)

## 0.9.2 (2026-06-28)

### Reverted — 方案 A（round 5/6 实测证明在 Claude Code 架构上不可达）

Round 5 (cache 装 0.9.1 但 session 不 reload) + Round 6 (用户重启 Claude Code) 决定性证明 **persona md `tools:` 字段对 host-level tool（如 `Skill`）不生效**。3/3 subagent 实测 tools = `Bash, Read, Edit, Write` (+ user 全局 MCP)，**永远不含 Skill**——无论 persona md 怎么写。

也就是说 0.9.0 / 0.9.1 添加的方案 A 在 Claude Code 架构层面**根本不可达**，是 no-op。继续保留只会误导 subagent 浪费一次 guaranteed-unavailable 的 invoke 尝试，且让阅读代码的人困惑"为何 Skill 永远 unavailable"。

**0.9.2 清理**：
- 删 3 个 persona md (`task-swarm-{coder,reviewer,validator}.md`) 顶部「## superpowers 集成」段
- 3 个 persona md `tools:` 字段去 `Skill`（回到 0.9.0 之前的状态）
- `_prompt.py` 的 `_subagent_skills_block()` 改名段标题为「## 开发纪律 (范式参考)」（原: 「## 开发纪律 (推荐 superpowers skill)」），介绍文字明确「**不要尝试 `Skill(...)` 调用**——Claude Code subagent 在当前架构下无 Skill tool 权限」+ 删 `调用模式: Skill('superpowers:<name>')` pseudo-code 行；改 skill 列表渲染从 `` `superpowers:<name>` —— ... `` 到 `**<name>** —— ...`（去掉 `superpowers:` 前缀，让"范式标识"语义更清晰）
- `_DEFAULT_SUBAGENT_SKILLS` dict 内容保留不变（4 skill 映射仍是 coder initial→TDD / coder p0-fix→systematic-debug / coder v-fix→两者 / reviewer→requesting-code-review / validator→verification-before-completion）

**为何不删 _DEFAULT_SUBAGENT_SKILLS / _subagent_skills_block 整体（方案 B）**：

方案 B（task.md 注入「开发纪律」段）实测**承担 superpowers 集成 90% 价值**。Round 4 16/16 + Round 5 3/3 subagent 全部 native fallback 0 差错，coder 真按 TDD 范式跑（先写 fail test → 实现 → green，mvn 11/0），reviewer 真用 evidence tag (`[contract]` / `[req:x.y]` / `[security]`)，validator 真 demand executable command (mvn test / tsc / curl)。**段内文字描述本身就把 skill 范式核心传达给 subagent，subagent 看了按指引跑**——不需要真 invoke skill 拿 self-content。0.9.2 改文字让 subagent 不再浪费 invoke 尝试，方案 B 价值保留。

### 回归

- `tests/test_prompt_superpowers_skills.py` 更新断言（去掉 `superpowers:` 前缀）+ 新增 2 case (`test_block_warns_against_skill_invoke` + `test_block_drops_skill_invoke_pseudo_code`) → 12/12 pass
- 全 task-swarm 测试 195/195 pass（_DEFAULT_SUBAGENT_SKILLS dict 内容未变，仅渲染文字变；persona md tools 去 Skill 不影响任何 Python 逻辑）

### 0.9.1 retrospective

0.9.1 修法（persona md tools 加 Skill）当时基于 round 4 finding "方案 A 被 tools 权限 block"，假设 1 是"Claude Code session 启动 freeze agent registry，0.9.1 装 cache + 重启即生效"。Round 6 用户真重启后实测假设 1 不成立——`tools:` 字段对 host-level tool 不生效，是更深的 Claude Code 架构限制。0.9.1 不破坏什么（tools 加个不识别的字段没副作用），但应该撤销避免文档误导。

## 0.9.1 (2026-06-28)

### Fixed — superpowers Skill tool 权限（round 4 实测发现 0.9.0 关键 bug）

Round 4 试跑 `ticket-tag-management` 16 个 subagent 全部 invoke `Skill('superpowers:*')` 返回 unavailable。根因：3 个 persona md (`task-swarm-coder.md` / `task-swarm-reviewer.md` / `task-swarm-validator.md`) frontmatter `tools:` 字段没有 `Skill`，Claude Code subagent 在工具权限层面无法调用 Skill tool。

**修法**：3 个 persona md `tools:` 字段尾部追加 `Skill`。

| persona | 0.9.0 tools | 0.9.1 tools |
|---|---|---|
| coder | Bash, Read, Edit, Write, Grep, Glob | Bash, Read, Edit, Write, Grep, Glob, **Skill** |
| reviewer | Bash, Read, Grep, Glob | Bash, Read, Grep, Glob, **Skill** |
| validator | Bash, Read, Grep, Glob | Bash, Read, Grep, Glob, **Skill** |

**影响**：方案 A（persona md「superpowers 集成」段）从 "block" 变 "可生效"。方案 B（task.md 「开发纪律」段注入）在 0.9.0 已完整生效，0.9.1 无变更。

soft-degrade 设计依然成立：superpowers 未装时 `Skill` tool 调用会失败，subagent 走 native fallback 按 task.md schema 执行（round 4 实测 16 个 subagent native fallback 0 差错）。

**回归**：现有 `tests/test_prompt_superpowers_skills.py` 11 个 case 全 pass（_prompt.py 渲染逻辑未变）。新增 persona md tools 检查无独立单测（frontmatter 字符串级修改，不引入逻辑）。

## 0.9.0 (2026-06-28)

### Added — superpowers 集成（方案 A+B；用户 round 3 后明确要求）

specode 自身已在 4 phase（requirements/design/execution/acceptance）调 superpowers skill（brainstorming/writing-plans/subagent-driven-development/executing-plans/verification-before-completion）+ native fallback。但 task-swarm 路径完全无 superpowers 集成（specode SKILL line 181 明示 "the task-swarm path does not use superpowers"，是早期设计选择 — subagent 自带 prompt 完成任务，superpowers fork 与 task-swarm fork scope 重叠风险）。

Round 1/2/3 试跑反复证明 **TDD / systematic-debugging / code review 范式 / verification 范式**这类纪律 skill 适合嵌入 subagent，不会与 task-swarm fork 冲突（subagent 内部用 Skill tool 调用 skill，不 fork 第二层 subagent）。

**方案 A（agents/*.md persona 顶部加段）**：
- `agents/task-swarm-coder.md` 加 superpowers 集成段：initial mode 调 `superpowers:test-driven-development`，v-fix mode 调 `superpowers:systematic-debugging`
- `agents/task-swarm-reviewer.md` 加段：调 `superpowers:requesting-code-review` 拿 review 范式
- `agents/task-swarm-validator.md` 加段：调 `superpowers:verification-before-completion` 拿验证范式
- 统一 disclaimer：superpowers 未装或调用失败 → 直接按 task.md schema 跑，**不打折扣**

**方案 B（_prompt.py 渲染 task.md 加段）**：
- 新增 `_subagent_skills_block(role, mode)` helper + `_DEFAULT_SUBAGENT_SKILLS` 字典
- coder/reviewer/validator 三个 render 函数都接 — task.md 在「项目级约束」段之后插入「## 开发纪律（推荐 superpowers skill）」段
- 段头硬约束 disclaimer：skill 是加速器不是 task.md 边界替代品；未装 silently degrade，task.md schema 仍是 SoT
- 调用模式 pseudo-code 在段尾给：`Skill('superpowers:<name>')` 失败走 except 分支不阻塞

**4 skill 矩阵**：

| role × mode | superpowers skill |
|---|---|
| coder initial | `test-driven-development` |
| coder p0-fix | `systematic-debugging` |
| coder v-fix | `systematic-debugging` + `test-driven-development` |
| reviewer any | `requesting-code-review` |
| validator any | `verification-before-completion` |

故意不选的 skill：
- `brainstorming` / `writing-plans` — scope 不适合（task-swarm 不做需求/design 阶段）
- `subagent-driven-development` / `executing-plans` — scope 直接冲突（superpowers 自带 fork 与 task-swarm fork 互相打架）

**Tests**: `tests/test_prompt_superpowers_skills.py` 11 case（每 role × mode skill 集 / soft dep disclaimer / 3 render 函数都注入 / unknown role empty fallback）。task-swarm 测试 183 → 194 全过。

**Round 4 试跑可同时 verify 4 个 skill 在 subagent 内被调用 + native fallback 是否真生效。**
