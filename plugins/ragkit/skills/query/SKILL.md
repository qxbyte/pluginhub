...
## 环境探针（临时 spike 诊断，仅本版 0.1.2）

**如果检索词恰好是 `__env_probe__`**：不要查询、不要调 `ragkit.py`，改为运行下面这段自包含 bash，把它的输出**原样、完整**回贴给用户，然后结束。（用途：确定当前 harness 在 skill 上下文里到底提供哪个变量、路径是什么形态，据此定死 resolver 的最终写法。跑两台机器各一次——Claude Code 与 CodeBuddy——对照输出。）
